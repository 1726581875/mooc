...
public class MonitorRecordService {
    ...
    public PageVO<MonitorRecordVO> findPageByCondition(MonitorRecordParam monitorRecordParam, Integer pageIndex, Integer pageSize){
        ...
        // 设置用户名 + 账号
        Map<Integer, MoocUser> userMap = moocUserService.getUserMap(monitorRecordList.stream().map(MonitorRecord::getTeacherId)
     ...
     * @return 如果找不到返回null
     */
    public MonitorRecord findById(Integer id){
     ...
     * @return 删除条数
     */
    public Integer deleteAllByIds(List<Integer> monitorRecordIdList){
